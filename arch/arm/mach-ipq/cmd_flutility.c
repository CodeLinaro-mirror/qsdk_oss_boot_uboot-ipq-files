...
#ifdef CONFIG_GPT_UPDATE_PARAMS
#ifdef CONFIG_DEFAULT_MMC_FLASH_SIZE
#define DEFAULT_MMC_FLASH_SIZE		CONFIG_DEFAULT_MMC_FLASH_SIZE
#else
#define DEFAULT_MMC_FLASH_SIZE		0xE90000
#endif
#ifdef CONFIG_DEFAULT_NOR_FLASH_SIZE
#define DEFAULT_NOR_FLASH_SIZE		CONFIG_DEFAULT_NOR_FLASH_SIZE
#else
...
int isvalid_appsbl_image(uintptr_t load_addr)
{
	...
	case SMEM_BOOT_NORGPT_FLASH:
		board_type = ipq_get_current_board_flash_config(sfi->flash_type);
...
static int write_to_flash(struct fl_info *fl)
{
	...
	case SMEM_BOOT_QSPI_NAND_FLASH:
		snprintf(runcmd, sizeof(runcmd),
			"nand erase 0x%x 0x%x && nand write 0x%x 0x%x 0x%x && ",
			offset, part_size,
			address, offset, file_size);
		break;
	case SMEM_BOOT_MMC_FLASH:
		snprintf(runcmd, sizeof(runcmd),
			"mmc erase 0x%x 0x%x && mmc write 0x%x 0x%x 0x%x && ",
			offset, part_size,
			address, offset, file_size);
		break;
...
static int fl_read(struct fl_info *fl)
{
	...
	case SMEM_BOOT_NORGPT_FLASH:
		snprintf(runcmd, sizeof(runcmd),
				"sf probe && sf read 0x%x 0x%x 0x%x ",
				 address, offset, part_size);
		break;
	default:
		printf("Invalid flash type\n");
...
int write_ubi_vol(char *ubi_vol_name, uint32_t load_addr, uint32_t file_size)
{
	...
	if (!strncmp(ubi_vol_name, "ubi_rootfs", UBI_VOL_NAME_MAX)) {
		snprintf(runcmd, sizeof(runcmd),
			"ubi remove rootfs_data && ubi remove %s &&"
			"ubi create %s 0x%x && ubi write 0x%x %s 0x%x &&"
...
static int prepare_nor_gpt(char *part_name, uint32_t *offset,
				uint32_t *part_size, uint32_t *file_size)
{
	uint32_t flash_size = ipq_smem_get_flash_size(0);
	uint32_t fsize;
	int ret = 0;
	...
	if (strncmp(NOR_GPT_PART_NAME, (const char *)part_name,
		...
		*offset = 0;
		if (bdinfo->nor_gpt_pte.gpt_pte) {
			free(bdinfo->nor_gpt_pte.gpt_pte);
			bdinfo->nor_gpt_pte.gpt_pte = NULL;
		}
#if defined(CONFIG_GPT_UPDATE_PARAMS)
		g_gptsel = GPT;
#endif
...
int do_flash(struct cmd_tbl *cmdtp, int flag, int argc, char * const argv[])
{
	...
	switch (flash_type) {
	case SMEM_BOOT_SPI_FLASH:
		ret = ipq_find_flash_by_name(part_name);
		switch (ret) {
		case 1:
			/*
			 * Nand flash
			 */
			ret = ipq_getpart_offset_size(part_name, &offset,
							&part_size);
			flash_type = SMEM_BOOT_QSPI_NAND_FLASH;
			break;
		case -1:
...
#ifdef CONFIG_IPQ_NAND
	case SMEM_BOOT_QSPI_NAND_FLASH:
		ret = ipq_getpart_offset_size(part_name, &offset, &part_size);
		if (ret) {
#ifdef CONFIG_CMD_UBI
			is_ubi = ubi_vol_present(part_name);
			if (is_ubi)
...
mmc:
	...
	if (unlikely(g_gptsel != -1)) {
		...
		if (!blk_dev) {
			printf("blk_desc uclass_id %d not found\n", uclass_id);
			return CMD_RET_FAILURE;
		}
...
redo_flash:
	...
	if (argc == 5) {
		...
		snprintf(runcmd, sizeof(runcmd),
			"imxtract 0x%x %s && flash %s %s",
			load_addr, file_name,
			part_name, argv[4]);
	} else if (argc == 4) {
		snprintf(runcmd, sizeof(runcmd),
			"imxtract 0x%x %s && flash %s",
			load_addr, file_name,
			part_name);
	}
	if (run_command(runcmd, 0) != CMD_RET_SUCCESS)
