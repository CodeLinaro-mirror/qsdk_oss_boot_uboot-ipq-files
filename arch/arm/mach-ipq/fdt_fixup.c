...
static LIST_HEAD(cal_config_list);
struct list_head *cal_list_head = &cal_config_list;
#endif
...
__weak void ipq_fdt_fixup_flash(void *blob)
{
	...
	for (i = 0; mmc_fixup[i].path; ++i) {
		if (fdt_path_offset(blob, mmc_fixup[i].path) > 0) {
			for (j = 0; j < mmc_fixup[i].ncount; ++j)
				parse_fdt_fixup(mmc_fixup[i].fixup[j], blob);
		}
	}
#endif
}
...
static void ipq_fdt_fixup_usb_dev_mode(void *blob)
{
	...
	for (i = 0; usb_fixup[i].path; ++i) {
		if (fdt_path_offset(blob, usb_fixup[i].path) > 0) {
			for (j = 0; j < usb_fixup[i].ncount; ++j)
				parse_fdt_fixup(usb_fixup[i].fixup[j], blob);
		}
	}
}
...
static void ipq_fdt_fixup_dload_disable(void *blob)
{
	...
	static const char * const del_node[] = {
		"uboot",
		"bootloader",
		"sbl",
		NULL
