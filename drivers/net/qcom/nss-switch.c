...
static void ppe_port_mux_set(phys_addr_t reg_base, struct port_info *port)
{
	...
	u8 id = port->id;
	u8 uniphy_type = port->uniphy_type;
					   ...
					   struct ipq_edma_txdesc_ring *txdesc_ring)
{
	...
	writel((u32)(txdesc_ring->dma & EDMA_RING_DMA_MASK),
	...
	writel((u32)(txdesc_ring->sdma & EDMA_RING_DMA_MASK),
	...
	writel((u32)(txdesc_ring->count & EDMA_TXDESC_RING_SIZE_MASK),
					   ...
					   struct ipq_edma_txcmpl_ring *txcmpl_ring)
{
	...
	writel((u32)(txcmpl_ring->dma & EDMA_RING_DMA_MASK),
	...
	writel((u32)(txcmpl_ring->count & EDMA_TXDESC_RING_SIZE_MASK),
					   ...
					   struct ipq_edma_rxdesc_ring *rxdesc_ring)
{
	...
	writel((u32)(rxdesc_ring->dma & EDMA_RING_DMA_MASK),
	...
	writel((u32)(rxdesc_ring->sdma & EDMA_RING_DMA_MASK),
					   ...
					   struct ipq_edma_rxfill_ring *rxfill_ring)
{
	...
	writel((u32)(rxfill_ring->dma & EDMA_RING_DMA_MASK),
...
static int ipq_eth_probe(struct udevice *dev)
{
	...
	int clk_itr, clk_cnt, ret, i, reg_val, configured = 0;
	phys_addr_t base;
	...
	for (clk_itr = 0 ; clk_itr < clk_cnt ; clk_itr++) {
		if (!strncmp(clk_names[clk_itr], "gcc_uniphy", 10)) {
			...
			if (ipq_uniphy && ipq_uniphy[uniphy_id].status == SKU_ENABLED) {
				clk_get_by_name(dev, clk_names[clk_itr], &clk);
				if (ret && ret != -ENOENT) {
	...
	for (i = 0; i < CONFIG_ETH_MAX_MAC; ++i) {
		...
		if (port->phyaddr != 0xFF) {
			if (!port->bus) {
				ret = uclass_get_device_by_ofnode(UCLASS_MDIO, port->pnode, &busdev);
				if (ret)
					continue;
...
static int ipq_eth_ofdata_to_platdata(struct udevice *dev)
{
	...
	for (i = 0; i < CONFIG_ETH_MAX_MAC; ++i) {
		...
		if (!dev_read_phandle_with_args(dev, phy_handle, NULL, 0, 0, &phandle_args)) {
			...
			if (ipq_uniphy && uniphy_id < CONFIG_ETH_MAX_UNIPHY) {
