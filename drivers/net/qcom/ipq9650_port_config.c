...
/* Port wrapper mode values used to select PMA TAP1 target */
#define PORT_WRAPPER_25GBASE_R		9	/* matches fal_port_interface_mode_t */
 ...
 *
 * Single implementation for all three PMA DFE modes, matching SSDK
 * adpt_jhppe_uniphy_pma_dfe_{hw_tuning,sw_tuning,disabled}_init().
				    ...
				    bool is_long, int dfe_mode)
{
	...
	csr_write(uniphy_index, CSR0_ADDR(QSERDES_COM_HSCLK_SEL_1_ADDRESS), 0x04); /* Multi-mode shared */
	...
	csr_write(uniphy_index, CSR0_ADDR(QSERDES_COM_CORECLK_DIV_MODE0_QP_ADDRESS), 0x0A); /* For PSGMII & QSGMII */
	...
	csr_write(uniphy_index, CSR0_ADDR(QSERDES_COM_LOCK_CMP1_MODE4_ADDRESS), 0xFF); /* For 12.5GPHY only */
	csr_write(uniphy_index, CSR0_ADDR(QSERDES_COM_LOCK_CMP2_MODE4_ADDRESS), 0x27); /* For 12.5GPHY only */
	...
	csr_write(uniphy_index, CSR0_ADDR(QSERDES_RX_EXT_RXEQ_CTRL5_ADDRESS), 0x16); /* Enable vga_dfe_gap_time */
	...
	if (dfe_mode == 0)
		csr_write(uniphy_index, CSR0_ADDR(QSERDES_RX_EXT_DFE_TRAIN_TIME_RATE4_ADDRESS), 0x28);
	...
	if (dfe_mode == 0)
		csr_write(uniphy_index, CSR0_ADDR(QSERDES_RX_EXT_DFE_TRAIN_TIME_RATE5_ADDRESS), 0x28);
	csr_write(uniphy_index, CSR0_ADDR(QSERDES_RX_EXT_VGA_DFE_GAP_TIME_RATE56_ADDRESS), 0x11);
	csr_write(uniphy_index, CSR0_ADDR(QSERDES_RX_EXT_DLL_FTUNE_CAL_TIME_RATE456_ADDRESS), 0x30);
	csr_write(uniphy_index, CSR0_ADDR(QSERDES_RX_EXT_UCDR_FASTLOCK_FO_GAIN_RATE6_ADDRESS), 0x1C);
	csr_write(uniphy_index, CSR0_ADDR(QSERDES_RX_EXT_UCDR_FASTLOCK_SO_GAIN_RATE6_ADDRESS), 0x06);
	csr_write(uniphy_index, CSR0_ADDR(QSERDES_RX_EXT_UCDR_FASTLOCK_COUNT_HIGH_RATE6_ADDRESS), 0x02);
	...
	if (dfe_mode == 0)
		csr_write(uniphy_index, CSR0_ADDR(QSERDES_RX_EXT_DFE_TRAIN_TIME_RATE6_ADDRESS), 0x28);
	...
	csr_write(uniphy_index, CSR0_ADDR(QSERDES_RX_EXT_DCC_CLK_EXTRA_DIV2_EN_RATE_CTRL0_ADDRESS), 0x40);
	...
	if (dfe_mode == 0) {
		...
		if (uniphy_mode == PORT_WRAPPER_25GBASE_R) /* 25GAUI mode */
			csr_write(uniphy_index, CSR0_ADDR(QSERDES_TX_EXT_TAP1CODE_TARGET_VAL_ADDRESS), 0x0A);
		else /* 12.5GPHY or USXGMII mode */
			csr_write(uniphy_index, CSR0_ADDR(QSERDES_TX_EXT_TAP1CODE_TARGET_VAL_ADDRESS), 0x05);
		csr_write(uniphy_index, CSR0_ADDR(QSERDES_TX_EXT_TAP1CODE_STEP_VAL_ADDRESS), 0x01);
		csr_write(uniphy_index, CSR0_ADDR(QSERDES_TX_EXT_TAP1CODE_STEP_TIME_ADDRESS), 0x0A);
	}
 ...
 * @is_long: true for long channel, false for short channel
 *
 * Matches SSDK adpt_jhppe_uniphy_pma_dfe_hw_tuning_init().
 ...
 * @is_long: true for long channel, false for short channel
 *
 * Matches SSDK adpt_jhppe_uniphy_pma_dfe_sw_tuning_init().
 ...
 * @is_long: true for long channel, false for short channel
 *
 * Matches SSDK adpt_jhppe_uniphy_pma_dfe_disabled_init().
