...
int ipq_bb_mdio_write(struct udevice *dev, int addr, int devad, int reg, u16 val)
{
	struct mdio_perdev_priv *pdata = dev_get_uclass_priv(dev);
...
static int ipq_bb_mdio_probe(struct udevice *dev)
{
	...
	ret = gpio_request_by_name(dev, "mdc-gpio", 0, &priv->mdc, GPIOD_IS_OUT);
	if (ret)
		return ret;

	ret = gpio_request_by_name(dev, "mdio-gpio", 0, &priv->mdio, GPIOD_IS_OUT);
	if (ret)
		return ret;

	bb_miiphy_buses[dev_idx].priv = priv;
	snprintf(bb_miiphy_buses[dev_idx].name,	sizeof(bb_miiphy_buses[dev_idx].name),
