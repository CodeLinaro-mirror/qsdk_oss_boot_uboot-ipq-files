...
static uint32_t geni_se_get_tx_fifo_depth(phys_addr_t base,
					uint32_t geni_se_version)
{
	uint32_t tx_fifo_depth;
	uint32_t tx_fifo_depth_msk = TX_FIFO_DEPTH_MSK;
...
static void geni_se_config_packing(phys_addr_t base, int bpw,
					bool msb_to_lsb,
					bool tx_cfg,
					bool rx_cfg)
{
...
static int qupv3_spi_claim_bus(struct udevice *dev)
{
	...
	writel(slave_plat->cs[0], priv->base + SE_SPI_DEMUX_SEL);
	if (priv->cs_high)
		writel(BIT(slave_plat->cs[0]),
...
static void geni_set_oversampling(struct udevice *dev)
{
	...
	phys_addr_t parent_base = dev_read_addr(parent_dev);

	priv->geni_se_version = readl(parent_base + QUP_HW_VER_REG);
